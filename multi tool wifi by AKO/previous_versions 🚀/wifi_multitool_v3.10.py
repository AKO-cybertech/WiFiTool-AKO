...
{BR}{PRI}
  ...
  ╚███╔███╔╝██║██║     ██║
   ╚══╝╚══╝ ╚═╝╚═╝     ╚═╝{RS}
{DIM}        v3.11  ·  By AKO  ·  Windows Edition{RS}"""
...
def build_page(page_idx):
    ...
    lines = [
        ...
        f"{BR}{PRI}  {t('menu_title')}{RS} {DIM}· v3.11 · By AKO{RS}",
        f"{DIM}  {chr(9552)*62}{RS}",
        f"",
        f"  {nav_b}     {dots}     {nav_n}",
        f"",
...
def change_ssid():
    ...
    import re as _re
    content = _re.sub(r"(<name>)" + _re.escape(old_name) + r"(</name>)", f"\g<1>{new_name}\g<2>", content)
...
def mac_changer():
    ...
    if choice == "v":
        ...
        if guid:
            reg_key = f"HKLM\SYSTEM\CurrentControlSet\Control\Class\{{4d36e972-e325-11ce-bfc1-08002be10318}}"
...
def full_report():
    ...
    lines = []
    lines.append(f"RAPPORT RESEAU — WiFi Multi-Tool v3.11 — By AKO")
...
def main():
    ...
    while True:
        ...
        if choice == "n":
            ...
            clear_banner()
            print(f"\n{BR}{PRI}  WiFi Multi-Tool v3.11 — By AKO{RS}")
