...
{BR}{PRI}
  ...
  ╚███╔███╔╝██║██║     ██║
   ╚══╝╚══╝ ╚═╝╚═╝     ╚═╝{RS}
{DIM}        v3.6  ·  By AKO  ·  Windows Edition{RS}"""
...
MENU = f"""
{DIM}  {'═'*62}{RS}
{BR}{PRI}  WIFI MULTI-TOOL{RS} {DIM}· v3.6 · By AKO{RS}
{DIM}  {'═'*62}{RS}
...
def send_message():
    ...
    if mode == "r":
        ...
        print(f"\n  {DIM}[s] Scanner les appareils du reseau  |  [m] Saisir l'IP manuellement{RS}\n")
        sub = input(f"  {ACC}Votre choix : {RS}").strip().lower()

        ip_dest = ""
        if sub == "s":
            if not SCAPY_OK:
                print(err("Scapy requis pour le scan : pip install scapy + Npcap"))
            else:
                ...
                if devices:
                    ...
                    for i, d in enumerate(devices, 1):
                        print(f"  {ACC}{i:<4}{RS}{PRI}{d['ip']:<19}{RS}{DIM}{d['mac']:<21}{RS}{ACC}{d['host']}{RS}")
                    print()
                    pick = input(f"  {ACC}Numero de l'appareil destinataire (0=annuler) : {RS}").strip()
                    if pick.isdigit() and int(pick) > 0:
                        idx = int(pick) - 1
                        if 0 <= idx < len(devices):
                            ip_dest = devices[idx]["ip"]
                            print(ok(f"Destinataire : {ip_dest}  ({devices[idx]['host']})"))
                        else:
                            print(err("Numero invalide.")); pause(); return
                    else:
                        return
                else:
                    print(warn("Aucun appareil detecte. Saisissez l'IP manuellement."))
...
def main():
    ...
    while True:
        ...
        if choice in ACTIONS:
            ...
            print(f"\n{BR}{PRI}  WiFi Multi-Tool v3.6 — By AKO{RS}")
